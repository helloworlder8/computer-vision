...
def img2label_paths(img_sp):
    ...
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_sp]
...
def check_dataset(data_str, autodownload=True):
    ...
    for k in "train", "val":
        if k not in data_dict:
            if k != "val" or "validation" not in data_dict:
                raise SyntaxError(
                    emojis(f"{data_str} '{k}:' key missing ❌.\n'train' and 'val' are required in all data_dict YAMLs.")
                )
            LOGGER.info("WARNING ⚠️ renaming data_dict YAML 'validation' key to 'val' to match YOLO format.")
    ...
    if "names" not in data_dict and "nc" not in data_dict: #名字类别至少二选一
        raise SyntaxError(emojis(f"{data_str} key missing ❌.\n either 'names' or 'nc' are required in all data_dict YAMLs."))
    if "names" in data_dict and "nc" in data_dict and len(data_dict["names"]) != data_dict["nc"]: #长度不相等
        raise SyntaxError(emojis(f"{data_str} 'names' length {len(data_dict['names'])} and 'nc: {data_dict['nc']}' must match."))
    ...
    # Resolve paths
    path = Path(extract_dir or data_dict.get("path") or Path(data_dict.get("yaml_file", "")).parent)  # data_str root
    if not path.is_absolute():
        path = (DATASETS_DIR / path).resolve() #DATASETS_DIR 这个负责到数据集
    ...
    def __init__(self, path="coco8.yaml", task_name="detect", autodownload=False):
        ...
        if self.task_name == "classify":
            ...
            try:
                ...
                data = check_dataset(yaml_path, autodownload)  # dict
