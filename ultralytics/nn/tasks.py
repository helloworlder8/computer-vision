...
class Detection_Model(Base_Model): #检测模型
    ...
    def __init__(self, model_dict, ch=3, nc=None, verbose=False):
        super().__init__()
    ...
    def _build_model(self, ch, nc, verbose):
        ...
        if isinstance(last_layer_model, Detect):
            ...
            self.stride = last_layer_model.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))]) #1 3 256 256
...
def attempt_load_weights(model_pts, device=None, inplace=True, fuse=False):
    ...
    # Update modules in the ensemble
    for m in ensemble.modules():
        if hasattr(m, "inplace"):
            m.inplace = inplace  # Set inplace attribute
...
def parse_model(model_str, ch, verbose=True):  # model_dict, input_channels(3)  模型序列化
...
if __name__ == "__main__":
    ...
    model_str = "ultralytics/cfg_yaml/test_model_yaml/ShuffleNetV4.1.yaml"
